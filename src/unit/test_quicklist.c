...
int test_quicklistTestMain(int argc, char **argv, int flags) {
    ...
    option_count = sizeof(options) / sizeof(*options);
    fill_count = (int)(sizeof(fills) / sizeof(*fills));
    return 0;
}
...
int test_quicklistInsertAfter1Element(int argc, char **argv, int flags) {
    ...
    for (int _i = 0; _i < option_count; _i++) {
        ...
        /* verify results */
        iter = quicklistGetIteratorEntryAtIdx(ql, 0, &entry);
        TEST_ASSERT_MESSAGE("Value 0 didn't match", strncmp((char *)entry.value, "hello", 5) == 0);
        ql_release_iterator(iter);

        iter = quicklistGetIteratorEntryAtIdx(ql, 1, &entry);
        TEST_ASSERT_MESSAGE("Value 1 didn't match", strncmp((char *)entry.value, "abc", 3) == 0);
        ql_release_iterator(iter);
        quicklistRelease(ql);
    }
    TEST_ASSERT(err == 0);
